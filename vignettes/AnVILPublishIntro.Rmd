...
## The `gcloud` SDK and `GCPtools`

It is necessary to have the [gcloud SDK][1] available to copy notebook
files to the workspace. Test availability with the `GCPtools` package:
...
```{r gcloudsettings, eval = isAnVILWS()}
gcloud_account()
gcloud_project()
```
...
Jupyter (`.ipynb`) notebooks uses [Quarto][2] software. It must be
...
`as_workspace()` invokes `as_notebook()` so this step
...
Some _R_ resources, e.g., [bookdown][3] sites, are not in
...
Publish the resources with `as_workspace()`:

```{r as_workspace, eval = FALSE}
as_workspace(
...
Current best practice is to use [quarto][2] for conversion of
.Rmd to ipynb. Quarto is available on the Bioconductor docker image,
or easily installed on Linux, macOS, or Windows. See the
[Quarto Get Started guide][4] for installation instructions.
